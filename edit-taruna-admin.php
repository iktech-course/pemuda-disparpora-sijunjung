	...
	<div class="row">
	 ...
	 <nav class="navbar navbar-dark" style="background-color: blue;">
          ...
          <div class="container mt-4">
            <form action = "edit-taruna-proses.php" method="POST">
            <?php 
                include 'koneksi.php';

                $id = $_GET['images'];
                $query = mysqli_query($koneksi, "SELECT * FROM `tb_taruna` WHERE images = '$id';");
            ...
            <input type="hidden" name="images" value ="<?= $data['images'] ?>">
                ...
                <div class="mb-3">
                    <label for="exampleInputPassword1" class="form-label">NARASI</label>
                    <input type="text" class="form-control" id="narasi" name="narasi" value="<?= $data['narasi']?>">
                </div>
